...
namespace SimpleCQRS.Infrastructure.InMemory
{
    public class Repository<T> : IRepository<T> where T : AggregateRoot, new() //shortcut you can do as you see fit with new()
    {
        ...
        public Event Map(Domain.Events.DomainEvent @event)
        {
            ...
            {
                return new Infrastructure.Events.InventoryItemDeactivated(itemsRemovedFromInventory.Id);
            }
            throw new Exception();
        }
        ...
        public Domain.Events.DomainEvent Map(Infrastructure.Event @event)
        {
            ...
            {
                return new Domain.Events.InventoryItemDeactivated(itemsRemovedFromInventory.Id);
            }
            throw new Exception();
        }
